...
SKIP = {
    ...
    "design.md", "README.md", "dist", "backup", "docs", "assets",
    "PW Video Player.html", "PW Video Player_files", ".agents",
...
if __name__ == "__main__":
    ...
    if should_build:
        build_target("chrome", is_firefox=False)
        build_target("firefox", is_firefox=True)
        build_target("edge", is_firefox=False)
        
        # Remove any old zip archives inside the dist directory
        ...
        if "--zip" in sys.argv:
            ...
            def zip_target(name):
                ...
                zip_file_path = os.path.join(DIST_DIR, f"pw-{name}-v{version}.zip")
            ...
            def zip_source():
                zip_file_path = os.path.join(DIST_DIR, f"pw-source-v{version}.zip")
                if os.path.exists(zip_file_path):
                    os.remove(zip_file_path)
                with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
